...
{
    Task<IEnumerable<CodeDescription>> GetAll(int listingId);
}
...
public class ListingAccessibilityRepository : IListingAccessibilityRepository
{
    ...
    public async Task<IEnumerable<CodeDescription>> GetAll(int listingId)
    {
        using var dbConn = _dbProvider.GetConnection();
        dbConn.Open();
        var query = "[dbo].[uspSiteListingAccessibilityRetrieve]";
