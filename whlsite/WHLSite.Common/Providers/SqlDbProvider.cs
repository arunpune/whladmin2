...
namespace WHLSite.Common.Providers;
...
public class SqlDbProvider : IDbProvider
{
    ...
    public SqlDbProvider(ILogger<SqlDbProvider> logger, IConfiguration configuration)
    {
        ...
        {
            _dbConnString = configuration.GetConnectionString("SiteDbConnString") ?? throw new ApplicationException("Unable to initialize data connection");
        }
        else
        {
