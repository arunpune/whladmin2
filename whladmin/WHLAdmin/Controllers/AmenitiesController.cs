...
public class AmenitiesController : Controller
{
    ...
    public AmenitiesController(ILogger<AmenitiesController> logger, IAmenitiesService amenitiesService, IMessageService messageService)
    {
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        _amenitiesService = amenitiesService ?? throw new ArgumentNullException(nameof(amenitiesService));
    ...
    public async Task<IActionResult> Index([FromHeader] string username)
    {
        ...
        try
        {
            var model = await _amenitiesService.GetData(requestId, correlationId, username);
            return View(model);
        }
    ...
    public IActionResult Add([FromHeader] string username)
    {
        ...
        try
        {
            var model = _amenitiesService.GetOneForAdd(requestId, correlationId);
    ...
    [HttpPost]
    [AdminRoleAuthorize("SYSADMIN,OPSADMIN,LOTADMIN")]
    public async Task<IActionResult> Add([FromHeader] string username, [FromForm] EditableAmenityViewModel model)
    {
        var correlationId = username ?? Guid.NewGuid().ToString();
        var requestId = Activity.Current?.Id ?? HttpContext?.TraceIdentifier ?? correlationId;
        _logger.LogDebug($"RequestId: {requestId}, CorrelationId: {correlationId}, Message: Started");
        ...
        try
        {
            var returnCode = await _amenitiesService.Add(requestId, correlationId, username, model);
    ...
    [HttpGet]
    [AdminRoleAuthorize("SYSADMIN,OPSADMIN,LOTADMIN")]
    public async Task<IActionResult> Edit([FromHeader] string username, [FromQuery] int amenityId)
    {
        var correlationId = username ?? Guid.NewGuid().ToString();
        var requestId = Activity.Current?.Id ?? HttpContext?.TraceIdentifier ?? correlationId;
        _logger.LogDebug($"RequestId: {requestId}, CorrelationId: {correlationId}, Message: Started");
        ...
        try
        {
            var model = await _amenitiesService.GetOneForEdit(requestId, correlationId, amenityId);
            if (model == null)
            {
                return NotFound();
            }
    ...
    [HttpPost]
    [AdminRoleAuthorize("SYSADMIN,OPSADMIN,LOTADMIN")]
    public async Task<IActionResult> Edit([FromHeader] string username, [FromForm] EditableAmenityViewModel model)
    {
        var correlationId = username ?? Guid.NewGuid().ToString();
        var requestId = Activity.Current?.Id ?? HttpContext?.TraceIdentifier ?? correlationId;
        _logger.LogDebug($"RequestId: {requestId}, CorrelationId: {correlationId}, Message: Started");
        ...
        try
        {
            var returnCode = await _amenitiesService.Update(requestId, correlationId, username, model);
            if (returnCode.Length > 0)
            {
                return BadRequest(new ErrorViewModel
                {
    ...
    [HttpPost]
    [AdminRoleAuthorize("SYSADMIN,OPSADMIN,LOTADMIN")]
    public async Task<IActionResult> Delete([FromHeader] string username, [FromQuery] int amenityId)
    {
        var correlationId = username ?? Guid.NewGuid().ToString();
        var requestId = Activity.Current?.Id ?? HttpContext?.TraceIdentifier ?? correlationId;
        _logger.LogDebug($"RequestId: {requestId}, CorrelationId: {correlationId}, Message: Started");
        ...
        try
        {
            var returnCode = await _amenitiesService.Delete(requestId, correlationId, username, amenityId);
            if (returnCode.Length > 0)
            {
                return BadRequest(new ErrorViewModel
                {
