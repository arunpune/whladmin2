DROP PROCEDURE IF EXISTS [dbo].[uspSiteUserUpdateAddressInfo];
GO
...
-- Description:	Update address information for an existing profile
-- Examples:
--	DECLARE @ErrorMessage VARCHAR(1000);
--	EXEC dbo.uspSiteUserUpdateAddressInfo @Username = 'USERNAME'
...
--	SELECT @ErrorMessage;
-- =============================================
CREATE PROCEDURE [dbo].[uspSiteUserUpdateAddressInfo]
...
BEGIN
	...
	BEGIN CATCH
		...
		SET @ErrorMessage = 'Failed to update Site User Address Information - ' + CONVERT(VARCHAR(20), ERROR_NUMBER()) + ': ' + ERROR_MESSAGE();
		SELECT -1;
