DROP PROCEDURE IF EXISTS [dbo].[uspLotteryDownloadResults];
GO
...
-- Description:	Download results of a given lottery
-- Examples:
--	EXEC dbo.uspLotteryDownloadResults @LotteryID = 1 (Retrieve One)
-- =============================================
CREATE PROCEDURE [dbo].[uspLotteryDownloadResults]
	@LotteryID		INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	WITH ApplicantsCTE AS (
		...
		WHERE HA.LotteryID = @LotteryID
			AND (APPL.RelationTypeCD <> 'SELF' AND APPL.CoApplicantInd <> 1)
		--ORDER BY HA.ApplicationID, ApplicantSortID
	)
		SELECT RIGHT(ISNULL(RTRIM(HA.LotteryNumber), '      '), 6) AS LotteryNumber, HA.LotteryID
			, HA.ApplicationID, HA.Username
		...
		JOIN dbo.tblLotteries L ON L.LotteryID = HA.LotteryID
		...
		WHERE HA.LotteryID = @LotteryID AND HA.StatusCD = 'ASSIGNED'
		ORDER BY HA.LotteryNumber ASC;
