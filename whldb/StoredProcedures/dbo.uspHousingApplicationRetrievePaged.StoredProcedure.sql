DROP PROCEDURE IF EXISTS [dbo].[uspHousingApplicationRetrievePaged];
GO
...
-- Description:	Retrieve a page of housing applications by listing ID, or a single one by application ID
-- Examples:
--	EXEC dbo.uspHousingApplicationRetrievePaged @ListingID = 1 (Retrieve All with default paging (1, 100))
--	EXEC dbo.uspHousingApplicationRetrievePaged @ListingID = 1, @PageNo = 1, @PageSize = 1000 (Retrieve All with specified paging)
--	EXEC dbo.uspHousingApplicationRetrievePaged @ListingID = 1, @SubmissionTypeCD = 'ONLINE', @PageNo = 1, @PageSize = 1000 (Retrieve Online submissions with specified paging)
--	EXEC dbo.uspHousingApplicationRetrievePaged @ListingID = 1, @StatusCD = 'WAITLIST', @PageNo = 1, @PageSize = 1000 (Retrieve waitlisted submissions with specified paging)
--	EXEC dbo.uspHousingApplicationRetrievePaged @ApplicationID = 1 (Retrieve One)
-- =============================================
CREATE PROCEDURE [dbo].[uspHousingApplicationRetrievePaged]
	@ListingID				INT = 0
...
BEGIN
	...
	BEGIN
		...
		SELECT A.ApplicationID
				...
				, A.GenderCD, MDGND.[Description] AS GenderDescription
				, A.DateOfBirth, A.Last4SSN
				, A.IDTypeCD, MDIT.[Description] AS IDTypeDescription, A.IDTypeValue, A.IDIssueDate
				, A.RaceCD, MDRAC.[Description] AS RaceDescription
				, A.EthnicityCD, MDETH.[Description] AS EthnicityDescription
				, A.Pronouns
				, A.StudentInd, A.DisabilityInd, A.VeteranInd
				, A.PhoneNumber, A.PhoneNumberExtn, A.PhoneNumberTypeCD, MDPHT.[Description] AS PhoneNumberTypeDescription
				, A.AltPhoneNumber, A.AltPhoneNumberExtn, A.AltPhoneNumberTypeCD, MDAPHT.[Description] AS AltPhoneNumberTypeDescription
				...
				, A.LeadTypeCD, MDLT.[Description] AS LeadTypeDescription, A.LeadTypeOther
			...
			, A.DisqualifiedInd, A.DisqualificationCD, A.DisqualificationOther, A.DisqualificationReason
	...
	WHERE A.ListingID = @ListingID
		AND (@SubmissionTypeCD = 'ALL' OR A.SubmissionTypeCD = @SubmissionTypeCD)
		AND (@StatusCD = 'ALL' OR A.StatusCD = @StatusCD)
		AND A.StatusCD <> 'DRAFT'
