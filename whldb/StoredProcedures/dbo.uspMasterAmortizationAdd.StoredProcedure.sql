DROP PROCEDURE IF EXISTS [dbo].[uspMasterAmortizationAdd];
GO
...
-- Description:	Add a new Amortization
-- Examples:
--	DECLARE @ErrorMessage VARCHAR(1000);
--	EXEC dbo.uspMasterAmortizationAdd @Rate = 2.0, @RateInterestOnly = 0.16667, @Rate10Year = 9.20135
...
--	SELECT @ErrorMessage;
-- =============================================
CREATE PROCEDURE [dbo].[uspMasterAmortizationAdd]
	@Rate				DECIMAL(7, 5)
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO dbo.tblMasterAmortizations (Rate, RateInterestOnly, Rate10Year, Rate15Year
			...
			VALUES (@Rate, @RateInterestOnly, @Rate10Year, @Rate15Year, @Rate20Year, @Rate25Year, @Rate30Year, @Rate40Year
						, @CreatedBy, GETDATE(), @CreatedBy, GETDATE(), 1);


		INSERT INTO dbo.tblAudit (EntityTypeCD, EntityID, Username, ActionCD, Note, [Timestamp])
			VALUES ('AMORTIZATION', CONVERT(VARCHAR(20), @Rate), @CreatedBy, 'ADD', 'Added Amortization', GETDATE());
	...
	BEGIN CATCH
		...
		SET @ErrorMessage = 'Failed to add Amortization - ' + CONVERT(VARCHAR(20), ERROR_NUMBER()) + ': ' + ERROR_MESSAGE();
		SELECT -1;
