DROP PROCEDURE IF EXISTS [dbo].[uspSiteUserRetrieveByEmailAddress];
GO
...
-- Description:	Retrieve a list of site users, or a single one by email address
-- Examples:
--	EXEC dbo.uspSiteUserRetrieveByEmailAddress (Retrieve All)
--	EXEC dbo.uspSiteUserRetrieveByEmailAddress @EmailAddress = 'ADDRESS' (Retrieve One)
-- =============================================
CREATE PROCEDURE [dbo].[uspSiteUserRetrieveByEmailAddress]
...
BEGIN
	...
	WHERE NULLIF(RTRIM(@EmailAddress), '') IS NULL OR U.EmailAddress = @EmailAddress
