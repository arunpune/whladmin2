DROP PROCEDURE IF EXISTS [dbo].[uspHousingApplicationRetrieveDuplicatesByName];
GO
...
-- Description:	Retrieve a list of potential duplicate applications by Listing ID and Name
-- Examples:
--	EXEC dbo.uspHousingApplicationRetrieveDuplicatesByName @ListingID = 1, @Name = 'JohnSmith'
-- =============================================
CREATE PROCEDURE [dbo].[uspHousingApplicationRetrieveDuplicatesByName]
	@ListingID	INT
	, @Name		VARCHAR(500)
AS
BEGIN
	...
	WITH DuplicateApplicationsCTE AS (
		SELECT A.ApplicationID, ISNULL(RTRIM(A.FirstName), '') + ' ' + ISNULL(RTRIM(A.LastName), '') AS [Name]
		...
		WHERE HA.ListingID = @ListingID
			...
			AND ISNULL(RTRIM(A.FirstName), '') + ISNULL(RTRIM(A.LastName), '') <> ''
	)
		...
		WHERE A.ListingID = @ListingID
			AND A.ApplicationID IN (SELECT DISTINCT ApplicationID FROM DuplicateApplicationsCTE WHERE [Name] = @Name)
