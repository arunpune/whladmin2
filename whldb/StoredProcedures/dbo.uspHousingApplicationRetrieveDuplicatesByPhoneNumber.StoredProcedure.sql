DROP PROCEDURE IF EXISTS [dbo].[uspHousingApplicationRetrieveDuplicatesByPhoneNumber];
GO
...
-- Description:	Retrieve a list of potential duplicate applications by Listing ID and Phone Number
-- Examples:
--	EXEC dbo.uspHousingApplicationRetrieveDuplicatesByPhoneNumber @ListingID = 1, @PhoneNumber = '1234567890'
-- =============================================
CREATE PROCEDURE [dbo].[uspHousingApplicationRetrieveDuplicatesByPhoneNumber]
	@ListingID			INT
...
BEGIN
	...
	WITH DuplicateApplicationsCTE AS (
		SELECT A.ApplicationID, 'PRIMARY' AS PhoneNumberTypeCD, ISNULL(RTRIM(A.PhoneNumber), '') + ISNULL(RTRIM(A.PhoneNumberExtn), '') AS PhoneNumber
		...
		UNION
		SELECT A.ApplicationID, 'ALTERNATE' AS PhoneNumberTypeCD, ISNULL(RTRIM(A.AltPhoneNumber), '') + ISNULL(RTRIM(A.AltPhoneNumberExtn), '') AS PhoneNumber
		FROM dbo.tblHousingApplicants A
		JOIN dbo.tblHousingApplications HA ON HA.ApplicationID = A.ApplicationID
		WHERE HA.ListingID = @ListingID
			AND HA.StatusCD IN ('SUBMITTED', 'WAITLISTED', 'ASSIGNED', 'DUPLICATE')
		...
		SELECT A.ApplicationID
				...
				, A.PhoneNumber, A.AltPhoneNumber
		...
		WHERE A.ListingID = @ListingID
			AND A.ApplicationID IN (SELECT DISTINCT ApplicationID FROM DuplicateApplicationsCTE WHERE PhoneNumber = @PhoneNumber)
