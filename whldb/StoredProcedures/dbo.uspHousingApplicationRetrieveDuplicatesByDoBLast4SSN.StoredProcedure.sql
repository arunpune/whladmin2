DROP PROCEDURE IF EXISTS [dbo].[uspHousingApplicationRetrieveDuplicatesByDoBLast4SSN];
GO
...
-- Description:	Retrieve a list of potential duplicate applications by Listing ID, Date of Birth and Last 4 of Tax ID / SSN
-- Examples:
--	EXEC dbo.uspHousingApplicationRetrieveDuplicatesByDoBLast4SSN @ListingID = 1, @DateOfBirth = '1980-01-01', @Last4SSN = '1234'
-- =============================================
CREATE PROCEDURE [dbo].[uspHousingApplicationRetrieveDuplicatesByDoBLast4SSN]
	@ListingID			INT
...
BEGIN
	...
	WITH DuplicateApplicationsCTE AS (
		SELECT A.ApplicationID, A.DateOfBirth, A.Last4SSN
		...
		WHERE HA.ListingID = @ListingID
			...
			AND (ISNULL(FORMAT(A.DateOfBirth, 'yyyy-MM-dd'), '0001-01-01') = ISNULL(@DateOfBirth, '2999-12-31') OR ISNULL(A.Last4SSN, '0000') = ISNULL(RTRIM(@Last4SSN), '9999'))
	)
		...
		WHERE A.ListingID = @ListingID
			AND A.ApplicationID IN (SELECT DISTINCT ApplicationID FROM DuplicateApplicationsCTE)
