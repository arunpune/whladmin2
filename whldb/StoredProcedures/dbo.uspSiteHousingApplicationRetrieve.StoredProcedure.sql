DROP PROCEDURE IF EXISTS [dbo].[uspSiteHousingApplicationRetrieve];
GO
...
-- Description:	Retrieve a list of housing applications by username, or a single one by application ID
-- Examples:
--	EXEC dbo.uspSiteHousingApplicationRetrieve @Username = 'USERNAME' (Retrieve All)
--	EXEC dbo.uspSiteHousingApplicationRetrieve @ListingID = 1 (Retrieve by listing)
--	EXEC dbo.uspSiteHousingApplicationRetrieve @ApplicationID = 1 (Retrieve One)
-- =============================================
CREATE PROCEDURE [dbo].[uspSiteHousingApplicationRetrieve]
...
BEGIN
	...
	IF ISNULL(@ApplicationID, 0) > 0
		...
		WHERE A.Username = @Username AND A.ApplicationID = @ApplicationID;
	...
	ELSE
		...
		WHERE A.Username = @Username
			AND (ISNULL(@ListingID, 0) = 0 OR A.ListingID = @ListingID)
