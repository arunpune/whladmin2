...
WHERE name = N'WHL_LOTTERY_HOURLYRUN';
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'WHL_LOTTERY_HOURLYRUN', 
		@enabled=0, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Run lottery for the HomeSeeker application listings that are eligible', 
		@category_name=N'WHL', 
		@owner_login_name=@JobUsername, @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run Lotteries that are eligible', 
		@step_id=1, 
		...
		@command=N'EXEC dbo.uspLotteryGenerator', 
		@database_name=N'WHL', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'WHL_HOURLY', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
